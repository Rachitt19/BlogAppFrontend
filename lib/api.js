...
export const followAPI = {
  followUser: async (userId) => {
    const res = await apiClient.post(`/follows/${userId}/follow`);
    return res.data;
  },

  unfollowUser: async (userId) => {
    const res = await apiClient.delete(`/follows/${userId}/follow`);
    return res.data;
  },
  ...
  checkFollowStatus: async (userId) => {
    const res = await apiClient.get(`/follows/${userId}/status`);
    return res.data;
  }
};
...
export const chatAPI = {
  ...
  getUnreadCount: async () => {
    const res = await apiClient.get('/chats/unread');
    return res.data;
  },

  markRead: async (chatId) => {
    const res = await apiClient.put(`/chats/${chatId}/read`);
    return res.data;
  },

  createGroupChat: async (name, users) => {
    const res = await apiClient.post('/chats/group', { name, users });
    return res.data;
  },

  updateGroup: async (chatId, data) => {
    const res = await apiClient.put(`/chats/group/${chatId}`, data);
    return res.data;
  },

  addGroupMember: async (chatId, userId) => {
    const res = await apiClient.put(`/chats/group/${chatId}/add`, { userId });
    return res.data;
  },

  removeGroupMember: async (chatId, userId) => {
    const res = await apiClient.put(`/chats/group/${chatId}/remove`, { userId });
    return res.data;
  },

  leaveGroup: async (chatId) => {
    const res = await apiClient.put(`/chats/group/${chatId}/leave`);
    return res.data;
  },
...
export const forumAPI = {
  getThreads: async (communityId, page = 1) => {
    const res = await apiClient.get(`/forums/community/${communityId}?page=${page}`);
    return res.data;
  },

  getThread: async (threadId) => {
    const res = await apiClient.get(`/forums/${threadId}`);
    return res.data;
  },

  createThread: async (communityId, title, content) => {
    const res = await apiClient.post('/forums', {
      communityId,
      title,
      content
    });
  ...
  addComment: async (threadId, content) => {
    const res = await apiClient.post(`/forums/${threadId}/comments`, { content });
    return res.data;
  }
};
